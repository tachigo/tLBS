...
aeEventLoop *aeCreateEventLoop(int setsize) {
    ...
    if ((eventLoop = (aeEventLoop *)zmalloc(sizeof(*eventLoop))) == nullptr) goto err;
    ...
    if (eventLoop->events == nullptr || eventLoop->fired == nullptr) goto err;
    eventLoop->setsize = setsize;
    eventLoop->lastTime = time(nullptr);
 ...
 * If there are no timers nullptr is returned.
 *
 * Note that's O(N) since time events are unsorted.
 * Possible optimizations (not needed by tLBS so far, but...):
...
void aeMain(aeEventLoop *eventLoop) {
    ...
    while (!eventLoop->stop) {
        if (eventLoop->beforesleep != nullptr)
            eventLoop->beforesleep(eventLoop);
        aeProcessEvents(eventLoop, AE_ALL_EVENTS|AE_CALL_AFTER_SLEEP);
    }
}

const char *aeGetApiName() {
    return aeApiName();
}
