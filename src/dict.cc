...
static int _dictInit(dict * ht, dictType * type, void * privateDataPtr);
 ...
 *
 * entry = dictAddRaw(dict,mykey,nullptr);
 ...
 * to dictUnlink(). It's safe to call this function with 'he' = nullptr. */
...
dictEntry *dictFind(dict *d, const void *key)
{
    ...
    if (d->ht[0].used + d->ht[1].used == 0) return nullptr; /* dict is empty */
    if (dictIsRehashing(d)) _dictRehashStep(d);
    h = dictHashKey(d, key);
    for (table = 0; table <= 1; table++) {
        idx = h & d->ht[table].sizemask;
 ...
 * return value is the reference to the dictEntry if found, or nullptr if not found. */
