 ...
 *
 * Both filename and options can be nullptr, in such a case are considered
...
//
//    if (dictAdd(state->rewritten,opt, nullptr) != DICT_OK) sdsfree(opt);
//}
...
//    if (fp == nullptr && errno != ENOENT) return nullptr;
//
//    char buf[CONFIG_MAX_LINE+1];
//    int linenum = -1;
//    struct rewriteConfigState *state = (rewriteConfigState *)zmalloc(sizeof(*state));
//    state->option_to_line = dictCreate(&optionToLineDictType,nullptr);
//    state->rewritten = dictCreate(&optionSetDictType,nullptr);
//    state->numlines = 0;
//    state->lines = nullptr;
//    state->has_tail = 0;
//    if (fp == nullptr) return state;
//
...
//    }
//    rewriteConfigStringOption(state,"dir",cwd,nullptr);
//}
...
//    const char *sep = " ";
//    addresses = sdsjoin(server.bindaddr,server.bindaddr_count, (char *)sep);
//    line = sdsnew(option);
//    line = sdscatlen(line, " ", 1);
...
//         * don't understand. */
//        if (dictFind(state->rewritten,option) == nullptr) {
...
static int numericConfigSet(typeData data, sds value, int update, char **err) {
    ...
    if (data.numeric.is_memory) {
        ...
        if (memerr || ll < 0) {
            *err = (char *)"argument must be a memory value";
            return 0;
        }
    } else {
        if (!string2ll(value, sdslen(value),&ll)) {
            *err = (char *)"argument couldn't be parsed into an integer" ;
            return 0;
        }
    }
...
static int isValidDBfilename(char *val, char **err) {
    if (!pathIsBaseName(val)) {
        *err = (char *)"dbfilename can't be a path, just a filename";
        return 0;
    }
    return 1;
}
...
static int isValidAOFfilename(char *val, char **err) {
    if (!pathIsBaseName(val)) {
        *err = (char *)"appendfilename can't be a path, just a filename";
        return 0;
    }
    return 1;
}
...
//    UNUSED(val);
//    UNUSED(prev);
//    return updateTlsCfg(nullptr, nullptr, err);
//}
//#endif  /* USE_OPENSSL */
...
//        createBoolConfig("rdbcompression", nullptr, MODIFIABLE_CONFIG, server.rdb_compression, 1, nullptr, nullptr),
//        createBoolConfig("rdb-del-sync-files", nullptr, MODIFIABLE_CONFIG, server.rdb_del_sync_files, 0, nullptr, nullptr),
//        createBoolConfig("activerehashing", nullptr, MODIFIABLE_CONFIG, server.activerehashing, 1, nullptr, nullptr),
//        createBoolConfig("stop-writes-on-bgsave-error", nullptr, MODIFIABLE_CONFIG, server.stop_writes_on_bgsave_err, 1, nullptr, nullptr),
//        createBoolConfig("dynamic-hz", nullptr, MODIFIABLE_CONFIG, server.dynamic_hz, 1, nullptr, nullptr), /* Adapt hz to # of clients.*/
//        createBoolConfig("lazyfree-lazy-eviction", nullptr, MODIFIABLE_CONFIG, server.lazyfree_lazy_eviction, 0, nullptr, nullptr),
//        createBoolConfig("lazyfree-lazy-expire", nullptr, MODIFIABLE_CONFIG, server.lazyfree_lazy_expire, 0, nullptr, nullptr),
//        createBoolConfig("lazyfree-lazy-server-del", nullptr, MODIFIABLE_CONFIG, server.lazyfree_lazy_server_del, 0, nullptr, nullptr),
//        createBoolConfig("lazyfree-lazy-user-del", nullptr, MODIFIABLE_CONFIG, server.lazyfree_lazy_user_del , 0, nullptr, nullptr),
//        createBoolConfig("repl-disable-tcp-nodelay", nullptr, MODIFIABLE_CONFIG, server.repl_disable_tcp_nodelay, 0, nullptr, nullptr),
//        createBoolConfig("repl-diskless-sync", nullptr, MODIFIABLE_CONFIG, server.repl_diskless_sync, 0, nullptr, nullptr),
//        createBoolConfig("gopher-enabled", nullptr, MODIFIABLE_CONFIG, server.gopher_enabled, 0, nullptr, nullptr),
//        createBoolConfig("aof-rewrite-incremental-fsync", nullptr, MODIFIABLE_CONFIG, server.aof_rewrite_incremental_fsync, 1, nullptr, nullptr),
//        createBoolConfig("no-appendfsync-on-rewrite", nullptr, MODIFIABLE_CONFIG, server.aof_no_fsync_on_rewrite, 0, nullptr, nullptr),
//        createBoolConfig("cluster-require-full-coverage", nullptr, MODIFIABLE_CONFIG, server.cluster_require_full_coverage, 1, nullptr, nullptr),
//        createBoolConfig("rdb-save-incremental-fsync", nullptr, MODIFIABLE_CONFIG, server.rdb_save_incremental_fsync, 1, nullptr, nullptr),
//        createBoolConfig("aof-load-truncated", nullptr, MODIFIABLE_CONFIG, server.aof_load_truncated, 1, nullptr, nullptr),
//        createBoolConfig("aof-use-rdb-preamble", nullptr, MODIFIABLE_CONFIG, server.aof_use_rdb_preamble, 1, nullptr, nullptr),
//        createBoolConfig("cluster-replica-no-failover", "cluster-slave-no-failover", MODIFIABLE_CONFIG, server.cluster_slave_no_failover, 0, nullptr, nullptr), /* Failover by default. */
//        createBoolConfig("replica-lazy-flush", "slave-lazy-flush", MODIFIABLE_CONFIG, server.repl_slave_lazy_flush, 0, nullptr, nullptr),
//        createBoolConfig("replica-serve-stale-data", "slave-serve-stale-data", MODIFIABLE_CONFIG, server.repl_serve_stale_data, 1, nullptr, nullptr),
//        createBoolConfig("replica-read-only", "slave-read-only", MODIFIABLE_CONFIG, server.repl_slave_ro, 1, nullptr, nullptr),
//        createBoolConfig("replica-ignore-maxmemory", "slave-ignore-maxmemory", MODIFIABLE_CONFIG, server.repl_slave_ignore_maxmemory, 1, nullptr, nullptr),
//        createBoolConfig("jemalloc-bg-thread", nullptr, MODIFIABLE_CONFIG, server.jemalloc_bg_thread, 1, nullptr, updateJemallocBgThread),
//        createBoolConfig("activedefrag", nullptr, MODIFIABLE_CONFIG, server.active_defrag_enabled, 0, isValidActiveDefrag, nullptr),
        createBoolConfig("syslog-enabled", nullptr, IMMUTABLE_CONFIG, server.syslog_enabled, 0, nullptr, nullptr),
//        createBoolConfig("cluster-enabled", nullptr, IMMUTABLE_CONFIG, server.cluster_enabled, 0, nullptr, nullptr),
//        createBoolConfig("appendonly", nullptr, MODIFIABLE_CONFIG, server.aof_enabled, 0, nullptr, updateAppendonly),
//        createBoolConfig("cluster-allow-reads-when-down", nullptr, MODIFIABLE_CONFIG, server.cluster_allow_reads_when_down, 0, nullptr, nullptr),
...
//        createStringConfig("cluster-announce-ip", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.cluster_announce_ip, nullptr, nullptr, NULL),
//        createStringConfig("syslog-ident", nullptr, IMMUTABLE_CONFIG, ALLOW_EMPTY_STRING, server.syslog_ident, "redis", nullptr, NULL),
//        createStringConfig("dbfilename", nullptr, MODIFIABLE_CONFIG, ALLOW_EMPTY_STRING, server.rdb_filename, "dump.rdb", isValidDBfilename, NULL),
...
//        createEnumConfig("repl-diskless-load", nullptr, MODIFIABLE_CONFIG, repl_diskless_load_enum, server.repl_diskless_load, REPL_DISKLESS_LOAD_DISABLED, nullptr, NULL),
        createEnumConfig("loglevel", nullptr, MODIFIABLE_CONFIG, loglevel_enum, server.verbosity, LL_NOTICE, nullptr,
                         nullptr),
//        createEnumConfig("maxmemory-policy", nullptr, MODIFIABLE_CONFIG, maxmemory_policy_enum, server.maxmemory_policy, MAXMEMORY_NO_EVICTION, nullptr, NULL),
//        createEnumConfig("appendfsync", nullptr, MODIFIABLE_CONFIG, aof_fsync_enum, server.aof_fsync, AOF_FSYNC_EVERYSEC, nullptr, NULL),

        /* Integer configs */
        createIntConfig("databases", nullptr, IMMUTABLE_CONFIG, 1, INT_MAX, server.dbnum, 16, INTEGER_CONFIG, nullptr, nullptr),
        createIntConfig("port", nullptr, IMMUTABLE_CONFIG, 0, 65535, server.port, 6379, INTEGER_CONFIG, nullptr, nullptr), /* TCP port. */
...
//        createIntConfig("tcp-keepalive", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.tcpkeepalive, 300, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("cluster-migration-barrier", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.cluster_migration_barrier, 1, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("active-defrag-cycle-min", nullptr, MODIFIABLE_CONFIG, 1, 99, server.active_defrag_cycle_min, 1, INTEGER_CONFIG, nullptr, NULL), /* Default: 1% CPU min (at lower threshold) */
//        createIntConfig("active-defrag-cycle-max", nullptr, MODIFIABLE_CONFIG, 1, 99, server.active_defrag_cycle_max, 25, INTEGER_CONFIG, nullptr, NULL), /* Default: 25% CPU max (at upper threshold) */
//        createIntConfig("active-defrag-threshold-lower", nullptr, MODIFIABLE_CONFIG, 0, 1000, server.active_defrag_threshold_lower, 10, INTEGER_CONFIG, nullptr, NULL), /* Default: don't defrag when fragmentation is below 10% */
//        createIntConfig("active-defrag-threshold-upper", nullptr, MODIFIABLE_CONFIG, 0, 1000, server.active_defrag_threshold_upper, 100, INTEGER_CONFIG, nullptr, NULL), /* Default: maximum defrag force at 100% fragmentation */
//        createIntConfig("lfu-log-factor", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.lfu_log_factor, 10, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("lfu-decay-time", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.lfu_decay_time, 1, INTEGER_CONFIG, nullptr, NULL),
...
//        createIntConfig("cluster-announce-port", nullptr, MODIFIABLE_CONFIG, 0, 65535, server.cluster_announce_port, 0, INTEGER_CONFIG, nullptr, NULL), /* Use server.port */
//        createIntConfig("repl-timeout", nullptr, MODIFIABLE_CONFIG, 1, INT_MAX, server.repl_timeout, 60, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("repl-ping-replica-period", "repl-ping-slave-period", MODIFIABLE_CONFIG, 1, INT_MAX, server.repl_ping_slave_period, 10, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("list-compress-depth", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.list_compress_depth, 0, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("rdb-key-save-delay", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.rdb_key_save_delay, 0, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("key-load-delay", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.key_load_delay, 0, INTEGER_CONFIG, nullptr, NULL),
//        createIntConfig("active-expire-effort", nullptr, MODIFIABLE_CONFIG, 1, 10, server.active_expire_effort, 1, INTEGER_CONFIG, nullptr, NULL), /* From 1 to 10. */
//        createIntConfig("hz", nullptr, MODIFIABLE_CONFIG, 0, INT_MAX, server.config_hz, CONFIG_DEFAULT_HZ, INTEGER_CONFIG, nullptr, updateHZ),
//        createIntConfig("min-replicas-to-write", "min-slaves-to-write", MODIFIABLE_CONFIG, 0, INT_MAX, server.repl_min_slaves_to_write, 0, INTEGER_CONFIG, nullptr, updateGoodSlaves),
//        createIntConfig("min-replicas-max-lag", "min-slaves-max-lag", MODIFIABLE_CONFIG, 0, INT_MAX, server.repl_min_slaves_max_lag, 10, INTEGER_CONFIG, nullptr, updateGoodSlaves),

        /* Unsigned int configs */
//        createUIntConfig("maxclients", nullptr, MODIFIABLE_CONFIG, 1, UINT_MAX, server.maxclients, 10000, INTEGER_CONFIG, nullptr, updateMaxclients),

        /* Unsigned Long configs */
//        createULongConfig("active-defrag-max-scan-fields", nullptr, MODIFIABLE_CONFIG, 1, LONG_MAX, server.active_defrag_max_scan_fields, 1000, INTEGER_CONFIG, nullptr, NULL), /* Default: keys with more than 1000 fields will be processed separately */
//        createULongConfig("slowlog-max-len", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.slowlog_max_len, 128, INTEGER_CONFIG, nullptr, NULL),
//        createULongConfig("acllog-max-len", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.acllog_max_len, 128, INTEGER_CONFIG, nullptr, NULL),

        /* Long Long configs */
//        createLongLongConfig("lua-time-limit", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.lua_time_limit, 5000, INTEGER_CONFIG, nullptr, NULL),/* milliseconds */
//        createLongLongConfig("cluster-node-timeout", nullptr, MODIFIABLE_CONFIG, 0, LLONG_MAX, server.cluster_node_timeout, 15000, INTEGER_CONFIG, nullptr, NULL),
//        createLongLongConfig("slowlog-log-slower-than", nullptr, MODIFIABLE_CONFIG, -1, LLONG_MAX, server.slowlog_log_slower_than, 10000, INTEGER_CONFIG, nullptr, NULL),
//        createLongLongConfig("latency-monitor-threshold", nullptr, MODIFIABLE_CONFIG, 0, LLONG_MAX, server.latency_monitor_threshold, 0, INTEGER_CONFIG, nullptr, NULL),
//        createLongLongConfig("proto-max-bulk-len", nullptr, MODIFIABLE_CONFIG, 0, LLONG_MAX, server.proto_max_bulk_len, 512ll*1024*1024, MEMORY_CONFIG, nullptr, NULL), /* Bulk request max size */
//        createLongLongConfig("stream-node-max-entries", nullptr, MODIFIABLE_CONFIG, 0, LLONG_MAX, server.stream_node_max_entries, 100, INTEGER_CONFIG, nullptr, NULL),
//        createLongLongConfig("repl-backlog-size", nullptr, MODIFIABLE_CONFIG, 1, LLONG_MAX, server.repl_backlog_size, 1024*1024, MEMORY_CONFIG, nullptr, updateReplBacklogSize), /* Default: 1mb */

        /* Unsigned Long Long configs */
//        createULongLongConfig("maxmemory", nullptr, MODIFIABLE_CONFIG, 0, ULLONG_MAX, server.maxmemory, 0, MEMORY_CONFIG, nullptr, updateMaxmemory),

        /* Size_t configs */
//        createSizeTConfig("hash-max-ziplist-entries", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.hash_max_ziplist_entries, 512, INTEGER_CONFIG, nullptr, NULL),
//        createSizeTConfig("set-max-intset-entries", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.set_max_intset_entries, 512, INTEGER_CONFIG, nullptr, NULL),
//        createSizeTConfig("zset-max-ziplist-entries", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.zset_max_ziplist_entries, 128, INTEGER_CONFIG, nullptr, NULL),
//        createSizeTConfig("active-defrag-ignore-bytes", nullptr, MODIFIABLE_CONFIG, 1, LLONG_MAX, server.active_defrag_ignore_bytes, 100<<20, MEMORY_CONFIG, nullptr, NULL), /* Default: don't defrag if frag overhead is below 100mb */
//        createSizeTConfig("hash-max-ziplist-value", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.hash_max_ziplist_value, 64, MEMORY_CONFIG, nullptr, NULL),
//        createSizeTConfig("stream-node-max-bytes", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.stream_node_max_bytes, 4096, MEMORY_CONFIG, nullptr, NULL),
//        createSizeTConfig("zset-max-ziplist-value", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.zset_max_ziplist_value, 64, MEMORY_CONFIG, nullptr, NULL),
//        createSizeTConfig("hll-sparse-max-bytes", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.hll_sparse_max_bytes, 3000, MEMORY_CONFIG, nullptr, NULL),
//        createSizeTConfig("tracking-table-max-keys", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.tracking_table_max_keys, 1000000, INTEGER_CONFIG, nullptr, NULL), /* Default: 1 million keys max. */

        /* Other configs */
//        createTimeTConfig("repl-backlog-ttl", nullptr, MODIFIABLE_CONFIG, 0, LONG_MAX, server.repl_backlog_time_limit, 60*60, INTEGER_CONFIG, nullptr, NULL), /* Default: 1 hour */
//        createOffTConfig("auto-aof-rewrite-min-size", nullptr, MODIFIABLE_CONFIG, 0, LLONG_MAX, server.aof_rewrite_min_size, 64*1024*1024, MEMORY_CONFIG, nullptr, NULL),

//#ifdef USE_OPENSSL
//    createIntConfig("tls-port", nullptr, IMMUTABLE_CONFIG, 0, 65535, server.tls_port, 0, INTEGER_CONFIG, nullptr, NULL), /* TCP port. */
//    createBoolConfig("tls-cluster", nullptr, MODIFIABLE_CONFIG, server.tls_cluster, 0, nullptr, NULL),
//    createBoolConfig("tls-replication", nullptr, MODIFIABLE_CONFIG, server.tls_replication, 0, nullptr, NULL),
//    createBoolConfig("tls-auth-clients", nullptr, MODIFIABLE_CONFIG, server.tls_auth_clients, 1, nullptr, NULL),
//    createBoolConfig("tls-prefer-server-ciphers", nullptr, MODIFIABLE_CONFIG, server.tls_ctx_config.prefer_server_ciphers, 0, nullptr, updateTlsCfgBool),
//    createStringConfig("tls-cert-file", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.cert_file, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-key-file", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.key_file, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-dh-params-file", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.dh_params_file, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-ca-cert-file", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.ca_cert_file, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-ca-cert-dir", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.ca_cert_dir, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-protocols", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.protocols, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-ciphers", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.ciphers, nullptr, nullptr, updateTlsCfg),
//    createStringConfig("tls-ciphersuites", nullptr, MODIFIABLE_CONFIG, EMPTY_STRING_IS_NULL, server.tls_ctx_config.ciphersuites, nullptr, nullptr, updateTlsCfg),
//#endif
...
//    if ((state = rewriteConfigReadOldFile(path)) == nullptr) return -1;
//
...
//    retval = rewriteConfigOverwriteFile((char *)server.configfile,newcontent);
//
//    sdsfree(newcontent);
    ...
    for (i = 0; i < totlines; i++) {
        ...
        for (standardConfig *config = configs; config->name != nullptr; config++) {
                 ...
                 (config->alias && !strcasecmp(argv[0],config->alias))))
            {
                ...
                if (!config->interface.set(config->data, argv[1], 0, (char **)&err)) {
                    goto loaderr;
                }
...
//            errno = 0;
//            server.unixsocketperm = (mode_t)strtol(argv[1], nullptr, 8);
...
//        }
            ...
            loadServerConfig(argv[1], nullptr);
        }
...
//    }
    ...
    loaderr:
    fprintf(stderr, "\n*** FATAL CONFIG FILE ERROR (tLBS %s) ***\n",
            TLBS_VERSION);
...
//        if (server.configfile == nullptr) {
//            addReplyError(c,"The server is running without a config file");
//            return;
//        }
//        if (rewriteConfig((char *)server.configfile) == -1) {
